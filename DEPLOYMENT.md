...
Registration is open by default — anyone who can reach the relay can create an
account. If that is not what you want, front it with whatever access control
you prefer; the relay has no invite mechanism.

```bash
curl -X POST https://relay.example.com/api/v2/auth/register/ \
  -H 'Content-Type: application/json' \
  -d '{"username":"you","password":"a-strong-password"}'
```

Username 3–50 characters, password at least 8. Or register from the Noo client
directly: *Preferences → Sync*, fill in the server URL, user name and server
password, then **Register**.

The admin dashboard is at `/admin/`, and logs in with `NOO_ADMIN_TOKEN`.
