...
which stores nothing; `507` a storage limit (quota, stream count, server disk).

The limits in `api/limits.go` apply to *new* data only — the next counter of a stream,
a blob not yet held. An already-held slot and a gap pass straight through to the
insert's own `200`/`409`, so a client can always re-send what the relay holds, however
full the account. A snapshot is credited with what its coverage frees, or an account at
its quota could never compact its way back under it.

Auth abuse answers `403` (register while `NOO_REGISTRATION` is closed, the default),
`429` with `Retry-After` (per-IP rate, per-username login backoff) and `503` (every
bcrypt slot busy). **`/auth/refresh` is never limited** — the client clears its tokens
when a refresh fails, so a 429 there would log devices out. The login backoff is keyed
on the username as sent, existing or not, so a 429 is no username oracle either.
...
| Account creation | Closed by default -- `POST /auth/register` answers 403 and the operator creates accounts from the dashboard. `NOO_REGISTRATION=open` opens it |
| Password guessing | Per-IP rate on login/register, per-username exponential backoff, bounded concurrent bcrypt. Client IPs come from `X-Forwarded-For` only via `NOO_TRUSTED_PROXIES` |
| Storage abuse | Per-account quota, stream cap, and a free-disk floor, all `507` |
...
- Run the tests before and after any change: `go test ./...` in `go/` (94 tests), and
...
- `newServer` in the tests opens registration and switches the auth rate limit and
  disk floor off, because nearly every test registers and logs in from httptest's one
  address. `limitServer` in `limits_test.go` keeps the shipped defaults; use it to test
