...
func newServer(t *testing.T) *Server {
	...
	// Most tests register their users and log in many times from httptest's
	// one fixed address. The limits have tests of their own.
	cfg.OpenRegistration = true
	cfg.AuthRatePerMinute = 0
	cfg.MinFreeDiskBytes = 0
	return newServerWith(t, cfg)
}

// newServerWith builds a relay from an explicit config, on a fresh database
// unless cfg already names one.
func newServerWith(t *testing.T, cfg *config.Config) *Server {
	t.Helper()
	if cfg.DatabaseURL == config.Default().DatabaseURL {
		cfg.DatabaseURL = filepath.Join(t.TempDir(), "test_noo_sync.db")
	}
