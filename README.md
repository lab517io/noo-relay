...
| `NOO_REGISTRATION` | `closed` | `open` lets anyone reaching the relay create an account; closed, accounts come from the admin dashboard |
| `NOO_USER_QUOTA_BYTES` | 0 (none) | Per-account storage limit, packets plus blobs |
| `NOO_MAX_STREAMS_PER_USER` | 256 | Distinct origin-device streams per account (0: no limit) |
| `NOO_MIN_FREE_DISK_BYTES` | 256 MB | Refuse uploads that would leave less than this free on the database's disk (0: off) |
| `NOO_AUTH_RATE_PER_MINUTE` | 20 | Login + register attempts per client IP (0: off). Refresh is never limited |
| `NOO_MAX_CONCURRENT_HASHES` | CPU count | bcrypt verifications in flight; beyond it, `503` after 10 s |
| `NOO_TRUSTED_PROXIES` | unset | IPs/CIDRs whose `X-Forwarded-For` names the client, e.g. `127.0.0.1,::1` behind a local proxy |
...
```bash
cd go && go test ./...        # 94 tests
cd go && go test ./... -race
```

94 tests covering registration, login, token refresh, packet upload/download,
...
size bounds, the refusal of weak secrets, closed registration, login rate limits and
backoff, and storage quotas.
...
`https://noo.lab517.io` is the instance this project runs; registration there is
closed, and its accounts are created by the operator.
